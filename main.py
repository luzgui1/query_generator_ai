...
def run_chatbot(user_input: str, memory_st, user_name: str, last_sql: str = "", last_preview: list = []):
    ...
    agentics = agents.GeminiAgents(model=model,schema=schema,user_name=user_name)
    ...
    if intention in ["analysis-request","correction-request"]:
        ...
        summary_response = agentics.general_response_builder(intent=intention,user_input=user_input,schema=schema,last_summary=preview_block,last_sql=last_sql)
